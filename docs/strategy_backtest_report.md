...
⚠️ **重要**：这比本地 yfinance prototype 低（那个 Sharpe 1.20）。差异在 Section 8 解释。

---

## 3b. 归因分析（CAPM / IR / Capture）

完整 attribution 表，2010-2026 全样本，196 个月度观察，benchmark = SPY：

| 指标 | 值 | 解读 |
|---|---|---|
| **CAPM Beta vs SPY** | **+0.330** | 只承担 SPY 33% 的市场风险 |
| **CAPM Alpha（年化）** | +8.01% | 跑赢 beta-implied 期望，但... |
| **Alpha t-stat** | 0.13 | **< 2.0 → alpha 不统计显著** |
| **R²** | 0.192 | 只 19% 变动跟 SPY 同向 — **策略基本 SPY-independent** |
| **Information Ratio** | -0.04 | 跟 SPY 当 benchmark **微弱跑输**（active return -0.5%/yr） |
| **Tracking Error** | 13.7% | 高——策略不"跟踪" SPY |
| **Sortino** | +1.28 | 只算下跌 vol → 比 Sharpe 0.99 高，**下跌时表现好** |
| **Calmar** | +0.84 | CAGR / |MDD| = 14% / 17%——good |
| **Treynor** | +0.35 | 单位 beta 回报，高（因 beta 低） |
| **Up Capture** | **0.557** | SPY 涨 10% 那个月我们涨 5.6% |
| **Down Capture** | **0.104** | SPY 跌 10% 那个月我们只跌 1% |
| **Capture Ratio** | **5.34** | **核心 trade-off**：每单位 upside 换 10 单位下跌豁免 |
| Skew | +0.21 | 轻微右偏（好） |
| Excess Kurtosis | +0.95 | 中等厚尾 |
| 月度 Hit Rate | 65.3% | 月度正收益占比 |
| 月度跑赢 SPY | 48.0% | **比 coin flip 还低**——策略不擅长赢 SPY，擅长少输 |
| 最连续下跌月数 | 5 | 心理承受 |
| 最大 drawdown 时长 | 18 个月 | 水下时间 |

### 关键解读

**Capture Ratio 5.34 是最重要的发现**。策略不是"打败 SPY"——是用**asymmetric downside protection** 换 upside 的一半。如果你的目标是"睡得着觉的 SPY 替代品"，这就是它最大的卖点。

**Alpha 不显著**：t-stat 0.13 远低于 2.0，**不能 statistically 证明 alpha 真实存在**。这跟 forward expectation 中的 Sharpe 0.5-0.7（已经打折）一致——backtest 看似 8% alpha，但样本里大半可能就是 capture 不对称带来的视觉效果。

**Beta 0.33** 解释为什么策略 vol 这么低（9.8% 年化）——大部分时间持有 bonds/gold（低 beta），权重才偏 equity。

### 年度 active return vs SPY

| 年份 | 策略 | SPY | Active | Strategy Sharpe |
|---|---|---|---|---|
| 2010 | +20.16% | +13.14% | **+7.03%** | +1.45 |
| 2011 | +13.69% | +0.85% | **+12.83%** | +1.21 |
| 2012 | +4.18% | +14.17% | -9.99% | +0.61 |
| **2013** | +0.01% | +29.00% | **-29.00%** | +0.04 |
| 2014 | +20.35% | +14.56% | +5.78% | +2.72 |
| 2015 | -0.17% | +1.29% | -1.46% | +0.03 |
| 2016 | +5.91% | +13.59% | -7.67% | +0.65 |
| 2017 | +22.32% | +20.78% | +1.54% | +2.22 |
| 2018 | -6.83% | -5.25% | -1.58% | -0.55 |
| 2019 | +19.52% | +31.09% | -11.57% | +2.28 |
| **2020** | +38.31% | +17.24% | **+21.07%** | +2.42 |
| 2021 | +15.31% | +30.51% | -15.19% | +0.95 |
| **2022** | -2.70% | -18.65% | **+15.95%** | -0.13 |
| 2023 | +18.72% | +26.71% | -7.99% | +1.43 |
| 2024 | +17.45% | +25.59% | -8.14% | +1.38 |
| 2025 | +28.95% | +18.01% | +10.94% | +2.36 |
| 2026* | +19.97% | +9.81% | +10.16% | +2.29 |

胜负：策略 8 / SPY 9。**最大差距 2013 -29%**（清晰单边牛市，策略持续在 bonds/gold 上 underperform）。**最大胜利 2020 +21% 和 2022 +16%**（COVID + 熊市，策略避险机制发威）。

**策略表现高度依赖 regime**：
- 长缓涨牛市（2013、2019、2021、2023、2024）：策略输 SPY 8-29 pp
- regime shift / 熊市（2020、2022）：策略胜 SPY 15-21 pp
